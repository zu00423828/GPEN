...
class FaceEnhancement(object):
    def __init__(self, base_dir='./', in_size=512, out_size=None, model=None, use_sr=True, sr_model=None, sr_scale=2, channel_multiplier=2, narrow=1, key=None, device='cuda'):
        ...
        self.facegan = FaceGAN(base_dir, in_size, out_size,
                               model, channel_multiplier, narrow, key, device=device)
    ...
    def process(self, img, aligned=False):
        ...
        for i, (faceb, facial5points) in enumerate(zip(facebs, landms)):
            ...
            of, tfm_inv = warp_and_crop_face(
                img, facial5points, reference_pts=self.reference_5pts, crop_size=(self.in_size, self.in_size))
        ...
        if self.use_sr and img_sr is not None:
            img = cv2.convertScaleAbs(
                img_sr*(1-full_mask) + full_img*full_mask)
        else:
